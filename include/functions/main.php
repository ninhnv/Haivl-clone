...
{
    global $config,$conn;
	$query = "select A.*, B.username from posts A, members B WHERE A.USERID=B.USERID AND A.PID!='".mysql_real_escape_string($pid)."' AND A.active='1' AND A.youtube_key='' AND A.fod_key='' order by rand() desc limit 5"; 
	$results = $conn->execute($query);
	$r = $results->getarray();
	return $r;
}
...
{	
	   ...
	   $check ="/^[[:alnum:]][a-z0-9_.-]*@[a-z0-9.-]+\.[a-z]{2,4}$/i";
       //return eregi($eregicheck, $emailtocheck);
	   return preg_match($check, $emailtocheck);
}
...
function insert_get_short_url($a)
{
	...
	$gagurl = $config['baseurl']."/photo/".$SPID;
	if($SPID > 0)
	{
		if($sshort == "")
		{
...
{
    $embedcode = '<iframe width="728" height="410" src="//www.youtube.com/embed/AWECDE" frameborder="0" allowfullscreen></iframe>';
	$item = $a[youtube];
...
//function insert_return_youtube($a)
//{
//    $embedcode = '<object style="background-image: url(http://i.ytimg.com/vi_webp/AWECDE/sddefault.webp);" width="728px" height="410px"><param name="movie" value="http://www.youtube.com/v/AWECDE&hl=en&fs=1"></param><param name="allowFullScreen" value="true"></param><embed src="http://www.youtube.com/v/AWECDE&hl=en&fs=1" type="application/x-shockwave-flash" allowfullscreen="true" width="728px" height="410px"></embed></object>';
//	$item = $a[youtube];
//	$embedcode = str_replace("AWECDE", $item, $embedcode);
//	return $embedcode;
//}
...
function insert_return_youtube3($a)
{
    $embedcode = '<div class="video-thumbnail"><target="_blank"><img data-src="http://img.youtube.com/vi/AWECDE/0.jpg" class="thumbImg" src="http://img.youtube.com/vi/AWECDE/0.jpg"><img src="http://hoyvl.net/images/play_icon1.png" class="videoIndicator" style="width:64px; height:64px"></a></div>';
	$item = $a[youtube];
	$embedcode = str_replace("AWECDE", $item, $embedcode);
	return $embedcode;
}
